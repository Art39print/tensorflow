...
// Add dot contraction dimensions in the order of lhs_contracting_dimensions.
bool TilingSpace::ProcessDot(const HloInstruction& hlo) {
  auto dot = DynCast<HloDotInstruction>(&hlo);
  if (!dot) {
    return false;
  }
  ...
  for (auto [index, contracting_dim_id] :
    ...
    AppendDimension(hlo, output_rank + index,
                    ...
                    DimensionType::kSequential);
  }
  return true;
}

// Add reduction dimensions.
bool TilingSpace::ProcessReduce(const HloInstruction& hlo) {
  auto reduce = DynCast<HloReduceInstruction>(&hlo);
  if (!reduce) {
    return false;
  }
...
bool TilingSpace::ProcessDynamicSlice(const HloInstruction& hlo) {
  auto ds = DynCast<HloDynamicSliceInstruction>(&hlo);
  if (!ds) {
    return false;
  }
...
std::string TilingSpace::ToString() const {
  ...
  for (const auto& dim : dimensions_) {
    ss << dim.id << " type: "
       << (dim.type == DimensionType::kParallel ? "parallel" : "sequential")
...
TilingSpace TilingSpace::Create(const HloFusionAdaptor& fusion) {
  ...
  for (auto it = post_order.rbegin(); it != post_order.rend(); ++it) {
    if (tiling_space.ProcessDot(it->instruction()) ||
