...
class TilingSpace {
 public:
  ...
  enum class DimensionType { kParallel, kSequential };
 ...
 private:
  void AppendDimension(const HloInstruction& hlo, int64_t dim_position,
                       int64_t dim_size, DimensionType dim_type);
  void AppendRTVar(const HloInstruction& hlo, const HloInstruction& rt_var,
                   int64_t upper_bound);

  bool ProcessDot(const HloInstruction& hlo);
  bool ProcessReduce(const HloInstruction& hlo);
  bool ProcessDynamicSlice(const HloInstruction& hlo);
